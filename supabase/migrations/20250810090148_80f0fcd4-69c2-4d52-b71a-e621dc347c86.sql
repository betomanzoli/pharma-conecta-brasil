-- Create ai_agent_outputs table to store outputs from all AI agents
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can insert own agent outputs'
      AND tablename = 'ai_agent_outputs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can view own agent outputs'
      AND tablename = 'ai_agent_outputs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Admins can view all agent outputs'
      AND tablename = 'ai_agent_outputs'
  ) THEN
